...
pub struct I2C_MultiRead;
pub struct I2C_MultiRead_PIO;

impl<P: OutputPin, T: I2c> DutTest<T, P> for I2C_MultiRead
where
    T::Error: defmt::Format,
{
    const S: TestSelector = TestSelector::I2C_MultiRead;
...
#[cfg(feature = "fp")]
impl<I: i2c::Instance, P: pio::Instance> FPTest<I, P> for I2C_MultiRead {
    const S: TestSelector = TestSelector::I2C_MultiRead;
...
#[cfg(feature = "fp")]
impl<I: i2c::Instance, P: pio::Instance> FPTest<I, P> for I2C_MultiRead_PIO {
    const S: TestSelector = TestSelector::I2C_MultiRead;
    ...
    async fn setup(
        ...
        pio.sm0.tx().push(u32::from_be_bytes(PAYLOAD));
    ...
    async fn run(
        ...
        for el in PAYLOAD {
            let _ = pio.sm0.rx().wait_pull().await;
        }
